...
public class Huffman {
	...
	private ArrayList<Integer> buildFrequencyTable(InputStream input) throws IOException{
		...
		byte[] freqList = input.readAllBytes();
		int totBytes = 0;

		for(byte index : freqList)
		{
			if(index<0) freqTable.set((index&0xFF), freqTable.get(index&0xFF)+1);
			else freqTable.set(index, freqTable.get(index)+1);
			totBytes++;
			if(index>255) break;
		}

		System.out.println("Number of bytes in input: " + totBytes);
	...
	private HuffmanTreeNode buildEncodingTree(ArrayList<Integer> freqTable) {
		...
		Integer index=0;
		for(int fc : freqTable)
		{
			if(fc>0){
				HuffmanTreeNode B = new HuffmanTreeNode(index, fc, null, null);
				P.insert(fc, B);
			}
			index++;
		}
		...
		while (P.size() > 1) {
			...
			T = new HuffmanTreeNode(0, newKey, e1.getValue(), e2.getValue());
			P.insert(newKey, T);
		}
	...
	private ArrayList<String> buildEncodingTable(HuffmanTreeNode encodingTreeRoot) {
		...
		ArrayList<HuffmanTreeNode> visited = new ArrayList<>();
		StringBuilder stringBuilder = new StringBuilder();
		HuffmanTreeNode current  = encodingTreeRoot;

		while(current!=null)
		{
			if(visited.contains(encodingTreeRoot)){
				current = null;
				break;
			}

			if(current.getLeft().isLeaf() && !(visited.contains(current.getLeft()))) {
				code.set(current.getLeft().getChar(), stringBuilder.toString()+"0");
				visited.add(current.getLeft());

				//System.out.println((char)current.getLeft().getChar() + ": " + stringBuilder.toString()+"0");
			}
			if(current.getRight().isLeaf() && !(visited.contains(current.getRight()))) {
				code.set(current.getRight().getChar(), stringBuilder.toString()+"1");
				visited.add(current.getRight());

				//System.out.println((char)current.getRight().getChar() + ": " + stringBuilder.toString()+"1");
			}

			if(visited.contains(current.getRight()) && visited.contains(current.getLeft())){
				visited.add(current);
			...
			}else if(!(current.getRight().isLeaf()) && !(visited.contains(current.getRight()))){
	...
	private void encodeData(InputStream input, ArrayList<String> encodingTable, OutputStream output) throws IOException {
		...
		while((in = input.read()) >= 0)
		{
			...
			String newByte = encodingTable.get(in);
			//System.out.println(newByte);
			for(String bit : newByte.split(""))
			{
				//System.out.print(bit);
				bitStream.writeBit(Integer.parseInt(bit));

				count++;
			}
			//System.out.println("");
		}
		System.out.println("Bytes in output: " + count/8);
	...
	private void decodeData(ObjectInputStream input, HuffmanTreeNode encodingTreeRoot, FileOutputStream output) throws IOException {
		...
		while((i=inputBitStream.readBit()) >= 0)
		{
			if(node!=null)
			{
				if (i == 1) node = node.getRight();
				else if (i == 0) node = node.getLeft();
		...
		System.out.println("bytes in input: " + count/8);
		System.out.println("bytes in output: " + cIn);
    }
